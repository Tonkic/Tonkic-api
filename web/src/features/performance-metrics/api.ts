...
import type {
  ...
  PerfSummaryAllData,
  PublicModelStatusData,
} from './types'

export async function getPublicModelStatus(): Promise<PublicModelStatusData> {
  const res = await api.get<PublicModelStatusData>('/api/status/models')
  return res.data
}
